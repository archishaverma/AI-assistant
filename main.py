...
activationWord = "teddy"#single word with which the assistant gets activated
...
# Wolfram Aplha client
appId = 'PRJW2Y-38Y9JA6PY2'
...
def parseCommand():
    ...
    with sr.Microphone() as source:
        listener.pause_threshold = 2#this is the gap before it cancels or ends the listening
...
def search_wolframAlpha(query = ''):
    ...
    # @numpods: Numeber of results reutrned
    # pod: List of results. This can also contain suppods
...
if __name__=='__main__':
    ...
    while True:
        ...
        #List commands
        if query[0] == 'say': #query[0] represents the first word you say
            if "hello" in query:
                speak("Hello, Archisha")
            else:
        ...
        if query[0] == 'log':
            ...
            with open('note.text' % now, 'w') as newFile:
                newFile.write(newNote)
            speak('Note written')

        #manners
        if query[0] == 'thank' or query[0] == 'thanks':
            speak('Anytime')
